	...
	var page = new Vue({
		...
		methods: {
			...
			fetchData (pageConfig) {
				...
				ajax.findPage('/redRedeemBillFinance', vm.searchParam).then(function (res) {
...
/** ************************侧滑栏**************************************** */
	...
	function showSlide(data) {
		...
		if (detailVue == null) {
			...
			ajax.detail('/redRedeemBillFinance', {
				...
				detailVue = new Vue({
					...
					methods: {
						...
						isPassOrNotPass(status){
							let vm = this
							vm.model['auditType'] = status
							ajax.post("/redRedeemBillFinance/updateState",vm.model).then(function (result) {
						...
						fetchData (index) {
							...
							if(index == 1){
								vm.loading = true
								ajax.findOne('/redRedeemBillFinance',vm.model.redeemBillId).then(function (res) {
									if (res.data) {
						...
						edit() {
							...
							util.dialog({
										...
										<template>
											<div class="ui-row">
												<div class="ui-form-group ui-col">
													<label class="ui-label-control ui-col-30">其他金额(元)</label>
													<div class="ui-col-70">
														 <el-input-number size="small" v-model="redRedeemBill.otherFee" controls-position="right" @change="change"></el-input-number>
													</div>
												</div>
											</div>
											<div class="ui-row">
												<div class="ui-form-group ui-col">
													<label class="ui-label-control ui-col-30">待付总金额</label>
													<div class="ui-col-70 ui-label-control" style="color: red">
														{{ redRedeemBill.realMoney }}
													</div>
												</div>
											</div>
										</template>
								...
								ok: function() {
									...
									ajax.update('/redRedeemBillFinance', editDialog.redRedeemBill).then(function (result) {
										if (result.state === 1) {
											util.dialog({getIdClose: 'edit-dialog'})
											detailVue.model.realMoney = editDialog.redRedeemBill.realMoney
											detailVue.model.otherFee = editDialog.redRedeemBill.otherFee
										}
									}, function () {
										// 使提交按钮可用
										util.setLoading({
								...
								onshow: function () {
									if (editDialog == null) {
										editDialog = new Vue({
											...
											methods: {
												change (val) {
													this.redRedeemBill.realMoney = val + this.data.realMoney
												}
											},
											mounted () {
												this.redRedeemBill.redeemBillId = this.data.redeemBillId
												this.redRedeemBill.realMoney = this.data.realMoney
											}
										})
									}
								}
