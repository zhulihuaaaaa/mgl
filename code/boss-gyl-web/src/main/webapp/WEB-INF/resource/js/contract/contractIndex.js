	...
	var page = new Vue({
		...
		methods: {
			...
			fetchData (pageConfig) {
				...
				ajax.findPage('/contract', vm.searchParam).then(function (res) {
			...
			content: `
					...
					<template>
						<div>
							...
							<div class="ui-row">
								...
								<div class="ui-col">
									<div class="ui-form-group ui-col-50">
										...
										<div class="ui-col-60">
											<component-select v-model="contract.entity.sellerId" :ignore-client-id="contract.entity.buyerId" type="PartyA" :search="true" name="buyerId">
                                         	</component-select>
										</div>
									</div>
									<div class="ui-form-group ui-col-50">
										...
										<div class="ui-col-60">
											<component-select class="sellerId" v-model="contract.entity.buyerId" :ignore-client-id="contract.entity.sellerId" type="crmClients" :search="true" :remoteseach="true" name="sellerId">
                                         	</component-select>
										</div>
									</div>
								</div>
								...
								<div class="ui-col">
									...
									<div class="ui-form-group ui-col-50">
										...
										<div class="ui-col-60">
											<component-upload @success="handleSuccess" ref="attach" :accept="accept" :multiple="true" @change="change"></component-upload>
										</div>
									</div>
								</div>
								<span  v-if="hiddenFee">
									<div class="ui-col">
										...
										<div class="ui-form-group ui-col-50">
											<label class="ui-label-control ui-col-30 required-txt">保证金金额(元)</label>
											<div class="ui-col-60">
												<input type="number" class="ui-input-control" v-model="depositFee" name="depositFee">
											</div>
										</div>
									</div>
									...
									<div class="ui-col">
										<div class="ui-form-group ui-col-50">
											...
											<div class="ui-col-60">
												<component-select v-model="contract.entity.invoiceBaseFee" type="invoice" name="invoiceBaseFee" :param="contract.entity.buyerId">
	                                         	</component-select>
											</div>
										</div>
									</div>
									<div class="ui-col">
										<div class="ui-form-group ui-col-50">
											...
											<div class="ui-col-60">
												<component-select v-model="contract.feeRates[0].feeValue" type="commission60" name="commission60" :param="contract.entity.buyerId">
	                                         	</component-select>
											</div>
										</div>
										<div class="ui-form-group ui-col-50">
											...
											<div class="ui-col-60">
												<component-select v-model="contract.feeRates[1].feeValue" type="commission90" name="commission90" :param="contract.entity.buyerId">
	                                         	</component-select>
											</div>
										</div>
									</div>
							...
							<h5 class="bg-tit">
								...
								<component-import  class="ui-btn pull-right btn_new_line"  @success="backtrack"  type="contract"></component-import>
							</h5>
							<div class="ui-row">
								<div class="ui-col">
									  <el-table 
				...
				ok: function () {
					if ($('#dialog-content').valid()) {
						if (tableContent.uploadImgTotal > 0) {
							tableContent.$refs.attach.submitUpload()
						} else {
							tableContent.contract.entity.depositFee = tableContent.depositFee
							// 禁用提交按钮
							util.setLoading({
								selector: '[i-id="ok"]'
							})
							if (type === 0) {
								ajax.save('/contract', tableContent.contract).then(function (result) {
									util.promptMsg({
								...
								ajax.update('/contract', tableContent.contract).then(function (result) {
									util.promptMsg({
										content: result.message
									})
									if (result.state === 1) {
								...
								'contract.contractDetail': {
									handler: function (newVal, oldVal) {
										...
										vm.$nextTick(function () {
											vm.contract.contractDetail.forEach(function (value, index) {
												...
												// 每行的价格
												//value.contractMoney = parseInt(value.netWeightDisplay == null ? 0 : value.netWeightDisplay) * parseInt(value.unitContractPrice == null ? 0 : value.unitContractPrice)
											...
											vm.totalMoney = totalMoney / 10000
											// vm.depositFee =  util.formatNumber((totalMoney / 10000 * vm.contract.entity.depositRate), 0, 'ceil')
											vm.totalWeight = totalWeight / 1000
											vm.contract.entity.depositFee = util.formatNumber((vm.contract.entity.depositRate * totalMoney), 0, 'ceil')
										})
									},
									deep: true
								},
								'contract.entity.buyerId': function (val, old) {
									this.contract.entity.invoiceBaseFee = ''
									this.contract.entity.commissionRate = ''
								},
								'uploadImgCount': function (val, old) {
									if (val === this.uploadImgTotal) {
										this.contract.entity.depositFee = this.depositFee * 10000
										if (this.type === 0) {
											ajax.save('/contract', this.contract).then(function (result) {
												util.promptMsg({
													content: result.message
												})
												if (result.state === 1) {
											...
											ajax.update('/contract', this.contract).then(function (result) {
												util.promptMsg({
													content: result.message
												})
												if (result.state === 1) {
							...
							methods: {
								...
								backtrack (res) {
									for (var row of res.data) {
										...
										this.$set(row,'netWeightUnit' , '')
										this.$set(row,'precision' , '')
										this.$set(row,'unitRate' , '')
								 ...
								 */
								copyLine (index, row) {
									this.contract.contractDetail.push($.extend(true, {}, row))
									contractValid()
								},
								/**
								 * 删除一行
								 ...
								 */
//								computedDepositFee() {
//									this.contract.entity.depositFee = util.formatNumber((this.contract.entity.depositRate * this.contract.entity.totalMoney), 0, 'ceil')
//								},
								/**
								 * 获取品类详情
								 */
									...
									if (id) {
										...
										ajax.findOne('/baseProduct', id).then(function (result) {
											if (result.state === 1) {
												...
												vm.contract.contractDetail[index].amountUnit = data.quantityUnit
												vm.contract.contractDetail[index].netWeightUnit = data.weightUnit
												vm.contract.contractDetail[index].precision = data.precision
												vm.contract.contractDetail[index].unitRate = data.unitRate
											}
										})
									}
								},
								getBaseInfo(){
									let vm = this
									ajax.post('/baseInfo/getGlobalBaseInfo').then(function (res) {
										console.log("res",res)
										vm.contract.entity.buyerId = res.data.companyName
									}, function () {
									})
								}
							},
