	...
	var page = new Vue({
		...
		methods: {
			...
			fetchData (pageConfig) {
				...
				ajax.findPage('/baseCost', vm.searchParam).then(function (res) {
			...
			del (row) {
				var vm = this
				ajax.delete('/baseCost', row.costId).then(function (result) {
					vm.fetchData()
				}, function () {
					vm.fetchData()
				})
			...
			content: `
				<form id="baseCostDialog">
					...
					<div class="ui-row">
						...
						<div class="ui-col">
							<div class="ui-form-group ui-col-50">
								<label class="ui-label-control ui-col-30 required-txt">0-60天手续费率</label>
								<div class="ui-col-60">
									<input type="text" 
									v-model="data.feeRate1" 
									class="ui-input-control" 
									name="feeRate1" 
									placeholder="请输入手续费率" 
									value="${type === 0 ? '' : data.feeRate1}">
								</div>
							</div>
						</div>
						<div class="ui-col">
							<div class="ui-form-group ui-col-50">
								<label class="ui-label-control ui-col-30 required-txt">61-90天手续费率</label>
								<div class="ui-col-60">
									<input type="text" 
									v-model="data.feeRate2" 
									class="ui-input-control" 
									name="feeRate2" 
									placeholder="请输入手续费率" 
									value="${type === 0 ? '' : data.feeRate2}">
								</div>
							</div>
						</div>
					</div>
				...
				ok: function () {
					if ($('#baseCostDialog').valid()) {
						...
						if (type === 0) {
							ajax.save('/baseCost', dialog.data).then(function (result) {
								util.promptMsg({
							...
							ajax.update('/baseCost', newValue).then(function (result) {
								util.promptMsg({
									content: result.message
								})
								util.dialog({getIdClose: 'baseCostDialog'})
	...
	function baseCostValid () {
		util.formValid({
			...
			messages: {
				...
				'feeRate1': {
					required: '请输入0-60天手续费数值',
					min: '精确度为0-1',
					max: '精确度为0-1',
					maxlength: '最多8位数字'
				},
				'feeRate2': {
					required: '请输入61-90天手续费数值',
					min: '精确度为0-1',
					max: '精确度为0-1',
					maxlength: '最多8位数字'
				}
