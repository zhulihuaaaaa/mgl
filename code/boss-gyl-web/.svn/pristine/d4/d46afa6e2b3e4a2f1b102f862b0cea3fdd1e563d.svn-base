	...
	var page = new Vue({
		...
		methods: {
			...
			fetchData (pageConfig) {
				...
				ajax.findPage('/financial', vm.searchParam).then(function (res) {
	...
	function showSlide(data) {
		...
		if (detail == null) {
			require(['component-slide','util'], function (componentSlide,util) {
				...
				ajax.detail('/financial/financial', {
					...
					detail = new Vue({
						...
						methods: {
							...
							findOneCon(){
								let vm =this
								let contractId =vm.model.conContract.contractId
							...
							passOrNotPass(status){
								...
								ajax.post('/financial/updatePassOrNoPass',one).then(function (res) {
										vm.tableConfig.loading = false
										vm.loading = false
					...
					newVue = new Vue({
						...
						methods: {
						  ...
						  Bathupdate(){
							  ...
							  this.format()
							  if(vm.isUpload){
								  ajax.update('/financial/bathprice',vm.insInspectionBillVo).then(function (result) {
				...
				onshow: function () {
					...
					addVue = new Vue({
						...
						methods: {
						  ...
						  updatePrice(){
							  let vm =this
							  ajax.update('/financial/price',vm.insInspectionBillVo).then(function (res) {
								}, function () {
				 ...
				 <template>
				    <component-table
				      ...
				      <el-table-column
				        ...
				        label="合同单价">
				        <template slot-scope="scope">   
				        <el-input-number size="small" v-model="scope.row.insInspectionDetail.contractUnitPrice" controls-position="right" :min="0" ></el-input-number>元/{{scope.row.baseProduct.weightUnit}}
						</template>
				      </el-table-column>
				       <el-table-column
				          width="200"
				        label="合同金额">
				        <template slot-scope="scope"> 
				        <el-input-number size="small" v-model="scope.row.insInspectionDetail.contractMoney" controls-position="right" :min="0" ></el-input-number>元/{{scope.row.baseProduct.weightUnit}}
						</template>
				      </el-table-column>
				      <el-table-column
				         width="200"
				        label="实际单价">
				        <template slot-scope="scope"> 
				        <el-input-number size="small" v-model="scope.row.insInspectionDetail.realUnitPrice" controls-position="right" :min="0"  @blur="chandge(scope.row)"></el-input-number>元/{{scope.row.baseProduct.weightUnit}}
						</template>
				      </el-table-column>
				      <el-table-column
				          width="200"
				        label="实际金额">
				        <template slot-scope="scope"> 
				        <el-input-number size="small" v-model="scope.row.insInspectionDetail.realMoney" controls-position="right" :min="0" ></el-input-number>元/{{scope.row.baseProduct.weightUnit}}
						</template>
				      </el-table-column>
				      <el-table-column
				          width="150"
