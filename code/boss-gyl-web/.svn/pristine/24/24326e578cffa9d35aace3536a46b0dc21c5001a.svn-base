	...
	var page = new Vue({
		...
		methods: {
			...
			fetchData (pageConfig) {
				...
				ajax.findPage('/warranty', vm.searchParam).then(function (res) {
				...
				ajax.detail('/warranty', {
					...
					detail = new Vue({
						...
						methods: {
							...
							del (row) {
								var vm = this
								ajax.delete('/settleEdit/detail', row.insInspectionDetail.inspectionDetailId).then(function (result) {
									 // vm.fetchData()
