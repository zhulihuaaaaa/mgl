							...
							<div class="swiper-slide">	
								<div class="ui-panel">
									<div class="ui-panel-body scroll-body" v-if="activeIndex == 1 && tabData[activeIndex]">
										<h5 class="bg-tit">
											<span>货物信息</span>
											<erp:privilege key="GYL_OUTSTOCKBILL_EDIT">
 											<input v-if="null != flow && flow.canSubmit" type="button" @click="edit" style="background-color:#5CB85C;color:#fff" class="ui-btn pull-right btn_new_line" value="编辑" />
											</erp:privilege>
										</h5>
										<template> 
										<el-table
